...
import { Sparkles, Key, CheckCircle2, X, Cpu, Bot, ArrowRight, ShieldCheck, Flame, Database, Cloud } from 'lucide-react';

export const GroqModal: React.FC = () => {
  const { showGroqModal, setShowGroqModal } = useAuth();
  const [activeTab, setActiveTab] = useState<'groq' | 'gemini' | 'firebase'>('groq');
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-xs animate-in fade-in duration-150">
      <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 max-w-lg w-full overflow-hidden">
        ...
        <div className="bg-gradient-to-r from-blue-50 to-indigo-50 px-6 py-3 border-b border-blue-100 flex items-center justify-between text-xs">
          <div className="flex items-center space-x-2">
            <span className="font-semibold text-blue-950">Primary:</span>
            <span className="font-mono bg-white px-2 py-0.5 rounded border border-blue-200 text-blue-700 font-bold">Groq AI</span>
          </div>
          <ArrowRight className="h-3.5 w-3.5 text-blue-400" />
          <div className="flex items-center space-x-2">
            <span className="font-semibold text-indigo-950">Auto-Fallback:</span>
            <span className="font-mono bg-white px-2 py-0.5 rounded border border-indigo-200 text-indigo-700 font-bold">Google Gemini</span>
          </div>
        </div>
        ...
        <div className="flex border-b border-slate-200 bg-slate-50 text-xs font-semibold">
          ...
          <button
            ...
            className={`flex-1 py-3 px-3 flex items-center justify-center space-x-1.5 border-b-2 transition-all ${
              activeTab === 'gemini'
