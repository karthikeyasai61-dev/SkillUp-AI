...
export async function getBacklog(req, res) {
    try {
        ...
        const tasks = await db.all('SELECT * FROM backlog_tasks WHERE user_id = ? ORDER BY CASE priority WHEN "Critical" THEN 1 WHEN "High" THEN 2 WHEN "Medium" THEN 3 ELSE 4 END, created_at DESC', userId);
...
export async function updateBacklogStatus(req, res) {
    try {
        const userId = req.userId;
        const { id } = req.params;
