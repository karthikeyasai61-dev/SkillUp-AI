...
import { generateAssessment, generateRecoveryPlan } from '../services/groqService.js';
...
export async function getBranchAssessment(req, res) {
    try {
        const userId = req.userId;
        const topic = req.query.topic || 'Core Engineering Calculations';
        const db = await getDb();
        ...
        await db.run(`INSERT INTO assessments (id, user_id, title, branch, topic, difficulty, assessment_type, questions_json, pass_score, status)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'Available')`, assessmentId, userId, assessment.title, branch, topic, assessment.difficulty, 'Technical Mastery', JSON.stringify(assessment.questions), 70);
        res.json({
            id: assessmentId,
...
export async function submitMilestone(req, res) {
    try {
        ...
        if (passed) {
            ...
            await db.run("UPDATE milestones SET status = 'Completed', score = ?, unlocked_stage = 1 WHERE id = ?", numScore, milestoneId);
            // Unlock next milestone if available
            const nextMonthNumber = milestone.month_number + 1;
            await db.run("UPDATE milestones SET status = 'Available', unlocked_stage = 1 WHERE user_id = ? AND month_number = ?", userId, nextMonthNumber);
            res.json({
                passed: true,
                score: numScore,
            ...
            const recoveryPlan = await generateRecoveryPlan(studentProfileData, milestone.title, numScore, weakTopics.length > 0 ? weakTopics : ['Foundational Problem Solving']);
            await db.run("UPDATE milestones SET status = 'Recovery', score = ?, recovery_plan = ? WHERE id = ?", numScore, JSON.stringify(recoveryPlan), milestoneId);
            res.json({
                passed: false,
                score: numScore,
