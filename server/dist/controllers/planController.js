...
export async function getSemesterPlan(req, res) {
    try {
...
export async function getWeeklyPlan(req, res) {
    try {
        ...
        const weeklyPlan = await db.get('SELECT * FROM weekly_plans WHERE user_id = ? AND week_number = ? ORDER BY created_at DESC LIMIT 1', userId, weekNumber) || await db.get('SELECT * FROM weekly_plans WHERE user_id = ? ORDER BY week_number DESC LIMIT 1', userId);
        if (!weeklyPlan) {
            res.status(404).json({ error: 'No weekly plan found' });
            return;
        }
        const tasks = await db.all('SELECT * FROM daily_tasks WHERE user_id = ? AND week_id = ? ORDER BY day_index ASC, created_at ASC', userId, weeklyPlan.id);
        res.json({
            weeklyPlan: {
                ...weeklyPlan,
...
export async function getTodayOverview(req, res) {
    try {
        ...
        const activeWeek = await db.get('SELECT * FROM weekly_plans WHERE user_id = ? ORDER BY week_number DESC LIMIT 1', userId);
        let allTasks = [];
        if (activeWeek) {
            allTasks = await db.all('SELECT * FROM daily_tasks WHERE user_id = ? AND week_id = ? ORDER BY day_index ASC', userId, activeWeek.id);
        }
        ...
        const backlogCountRow = await db.get("SELECT COUNT(*) as count FROM backlog_tasks WHERE user_id = ? AND status = 'Pending'", userId);
        const backlogCount = backlogCountRow?.count || 0;
        // Next milestone
        const nextMilestone = await db.get("SELECT * FROM milestones WHERE user_id = ? AND status != 'Completed' ORDER BY month_number ASC LIMIT 1", userId);
    ...
    try {
        ...
        if (status === 'Missed') {
            ...
            if (!existingBacklog) {
                ...
                await db.run(`INSERT INTO backlog_tasks (id, user_id, original_task_id, title, topic, duration_minutes, priority, reason, status)
           VALUES (?, ?, ?, ?, ?, ?, 'High', 'Marked missed by student during daily schedule', 'Pending')`, backlogId, userId, taskId, task.title, task.topic, task.duration_minutes);
            }
        }
        else if (status === 'Completed') {
            // If was in backlog, mark backlog completed
            await db.run("UPDATE backlog_tasks SET status = 'Completed' WHERE original_task_id = ? AND user_id = ?", taskId, userId);
        }
