...
import { analyzeCurriculum, generatePersonalizedCurriculum, generateSemesterPlan, generateWeeklySchedule } from '../services/groqService.js';
import { firestoreSaveProfile, firestoreSaveCurriculum, firestoreSavePersonalizedCurriculum, firestoreSaveSemesterPlan, firestoreSaveWeeklyPlan, firestoreSaveDailyTasks, firestoreSaveMilestone } from '../services/firebaseService.js';
...
export async function saveProfile(req, res) {
    try {
        ...
        const db = await getDb();
        await db.run(`INSERT INTO student_profiles (user_id, branch, year_semester, career_goal, interests, current_skills, daily_learning_minutes, updated_at)
       ...
       ON CONFLICT(user_id) DO UPDATE SET
         ...
         updated_at=CURRENT_TIMESTAMP`, userId, branch, year_semester, career_goal, typeof interests === 'string' ? interests : JSON.stringify(interests || []), typeof current_skills === 'string' ? current_skills : JSON.stringify(current_skills || []), resolvedMinutes);
...
export async function uploadAndEvolveCurriculum(req, res) {
    try {
        const userId = req.userId;
        ...
        if (!profile && profileData) {
            ...
            await db.run(`INSERT INTO student_profiles (user_id, branch, year_semester, career_goal, interests, current_skills, daily_learning_minutes)
         VALUES (?, ?, ?, ?, ?, ?, ?)`, userId, profileData.branch || branch || 'Mechanical Engineering', profileData.year_semester || semester || 'Semester 5', profileData.career_goal || 'Industry Engineer', typeof profileData.interests === 'string' ? profileData.interests : JSON.stringify(profileData.interests || []), typeof profileData.current_skills === 'string' ? profileData.current_skills : JSON.stringify(profileData.current_skills || []), minutes);
        ...
        await db.run(`INSERT INTO curricula (id, user_id, branch, raw_text, file_name, analyzed_json)
       VALUES (?, ?, ?, ?, ?, ?)`, curriculumId, userId, analysis.branch || profile.branch, rawText, fileName || 'syllabus.txt', JSON.stringify(analysis));
        ...
        await db.run(`INSERT INTO personalized_curricula (id, user_id, keep_topics, compress_topics, add_topics, analysis_summary)
       VALUES (?, ?, ?, ?, ?, ?)`, personalizedId, userId, JSON.stringify(evolved.keep), JSON.stringify(evolved.compress), JSON.stringify(evolved.add), evolved.summary);
        ...
        await db.run(`INSERT INTO semester_plans (id, user_id, total_weeks, roadmap_json)
       VALUES (?, ?, ?, ?)`, semesterPlanId, userId, semesterPlan.totalWeeks || 16, JSON.stringify(semesterPlan));
        ...
        await db.run(`INSERT INTO weekly_plans (id, user_id, week_number, title, focus_areas, adaptive_notes, status)
       VALUES (?, ?, ?, ?, ?, ?, 'active')`, weekId, userId, 1, weeklySchedule.title, JSON.stringify(week1Topics), weeklySchedule.adaptiveNotes);
        ...
        for (const day of weeklySchedule.days) {
            for (const task of day.tasks) {
                ...
                await db.run(`INSERT INTO daily_tasks (id, user_id, week_id, day_name, day_index, title, topic, task_type, duration_minutes, status, date_str)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'Not Started', ?)`, taskId, userId, weekId, day.dayName, day.dayIndex, task.title, task.topic, task.taskType, task.durationMinutes, dateStr);
                createdTasks.push({
                    id: taskId,
                    week_id: weekId,
        ...
        if (semesterPlan.months && semesterPlan.months.length > 0) {
            for (const month of semesterPlan.months) {
                ...
                await db.run(`INSERT INTO milestones (id, user_id, month_number, title, description, required_topics, status, unlocked_stage)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, milestoneId, userId, month.month, month.milestoneTitle || `Month ${month.month} Mastery Milestone`, month.focus || `Validation of Month ${month.month} skills`, JSON.stringify(month.weeks?.flatMap((w) => w.keyTopics || []) || []), month.month === 1 ? 'Available' : 'Locked', month.month === 1 ? 1 : 0);
