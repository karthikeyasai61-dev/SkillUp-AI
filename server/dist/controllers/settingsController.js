...
export async function getFirebaseStatus(req, res) {
    res.json({
        connected: true,
        projectId: 'skillup-ai-aa9ab',
        service: 'Google Cloud Firebase Firestore',
...
export async function saveGroqKey(req, res) {
    try {
        ...
        const db = await getDb();
        await db.run(`INSERT INTO system_settings (key, value) VALUES ('GROQ_API_KEY', ?)
       ON CONFLICT(key) DO UPDATE SET value = excluded.value`, trimmed);
...
export async function saveGeminiKey(req, res) {
    try {
        const { apiKey } = req.body;
        if (!apiKey || typeof apiKey !== 'string' || apiKey.trim().length < 8) {
        ...
        const db = await getDb();
        await db.run(`INSERT INTO system_settings (key, value) VALUES ('GEMINI_API_KEY', ?)
       ON CONFLICT(key) DO UPDATE SET value = excluded.value`, trimmed);
