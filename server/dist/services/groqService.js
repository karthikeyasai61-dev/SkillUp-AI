...
export async function getActiveGroqModel(groq) {
    if (cachedGroqModel)
...
async function callAiWithJsonFormat(prompt, temperature = 0.2) {
    let lastErrorMsg = '';
    ...
    throw new Error(`AI generation failed. Please configure your Groq API Key or Google Gemini API Key in Settings. (Details: ${lastErrorMsg})`);
}
// -------------------------------------------------------------
// 1. ANALYZE CURRICULUM (100% AI Extracted - NO Pre-existing Data)
// -------------------------------------------------------------
export async function analyzeCurriculum(rawText, branchHint, semesterHint) {
...
${rawText.slice(0, 15000)}`;
    return callAiWithJsonFormat(prompt, 0.1);
}
// -------------------------------------------------------------
// 2. EVOLVE CURRICULUM (KEEP / COMPRESS / ADD)
// -------------------------------------------------------------
export async function generatePersonalizedCurriculum(profile, curriculumData) {
...
Return a valid JSON object matching EXACTLY this structure:
{
  ...
  "add": [
    ...
    return callAiWithJsonFormat(prompt, 0.2);
}
// -------------------------------------------------------------
// 3. GENERATE SEMESTER ROADMAP (Semester -> Month -> Week)
// -------------------------------------------------------------
export async function generateSemesterPlan(profile, evolvedCurriculum) {
...
// -------------------------------------------------------------
export async function generateWeeklySchedule(profile, weekNumber, theme, topics, backlogTasks = []) {
...
// -------------------------------------------------------------
export async function generateAssessment(branch, topic, difficulty = 'Intermediate') {
...
// -------------------------------------------------------------
export async function adaptNextWeekPlan(profile, completedTasks, missedTasks, currentBacklog, currentWeekNumber, upcomingTopics) {
...
Return valid JSON:
{
  ...
  "nextWeekSchedule": {
    ...
    return callAiWithJsonFormat(prompt, 0.3);
}
// -------------------------------------------------------------
// 7. MONTHLY MILESTONE RECOVERY PLAN GENERATOR
// -------------------------------------------------------------
export async function generateRecoveryPlan(profile, milestoneTitle, failedScore, weakAreas) {
